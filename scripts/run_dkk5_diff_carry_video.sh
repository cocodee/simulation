...
RUN_DIR="${RUN_DIR:-$ROOT_DIR/outputs/dkk_5_diff_carry_${RUN_STAMP}}"
OUTPUT_DIR="${OUTPUT_DIR:-$RUN_DIR/frames}"
VIDEO_PATH="${VIDEO_PATH:-$RUN_DIR/dkk_5_diff_carry_video.mp4}"
MANIFEST_PATH="${MANIFEST_PATH:-$RUN_DIR/dkk_5_diff_carry_manifest.json}"
FRAMES="${FRAMES:-480}"
WARMUP_STEPS="${WARMUP_STEPS:-30}"
FPS="${FPS:-24}"
SETTLE_SECONDS="${SETTLE_SECONDS:-5}"
ARM_TRANSITION_SECONDS="${ARM_TRANSITION_SECONDS:-4}"
WIDTH="${WIDTH:-1280}"
HEIGHT="${HEIGHT:-720}"
...
WHEEL_SPEED="${WHEEL_SPEED:-2.5}"
ANGULAR_SPEED="${ANGULAR_SPEED:-0.0}"
WHEEL_RADIUS="${WHEEL_RADIUS:-0.075}"
WHEEL_BASE="${WHEEL_BASE:-0.6284}"
MAX_WHEEL_SPEED="${MAX_WHEEL_SPEED:-4.5}"
LEFT_CARRY_POSE="${LEFT_CARRY_POSE:-0.55,-0.42,0.30,-1.10,0.08,0.10}"
RIGHT_CARRY_POSE="${RIGHT_CARRY_POSE:--0.55,-0.42,-0.30,1.10,-0.08,-0.10}"
FINGER_CARRY_POSITION="${FINGER_CARRY_POSITION:-0.004}"
...
env -u CONDA_PREFIX \
  "$ISAAC_SIM_DIR/python.sh" "$ROOT_DIR/scripts/record_dkk5_diff_carry_video.py" \
    --scene="$SCENE_PATH" \
    ...
    --arm-transition-seconds="$ARM_TRANSITION_SECONDS" \
    --width="$WIDTH" \
    --height="$HEIGHT" \
    ...
    --left-carry-pose="$LEFT_CARRY_POSE" \
    --right-carry-pose="$RIGHT_CARRY_POSE" \
    --finger-carry-position="$FINGER_CARRY_POSITION" \
    "$MULTI_GPU_FLAG" \
